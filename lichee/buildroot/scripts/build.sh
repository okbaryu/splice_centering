...
build_buildroot()
{
    if [ ! -f ${LICHEE_BR_OUT}/.config ] ; then
        printf "\nUsing default config: ${LICHEE_BR_DEFCONF} ...\n\n"
...
build_toolchain()
{
    ...
    if [ -f ${tooldir}/.installed ] ; then
        ...
        tar --strip-components=1 \
            -jxf ${LICHEE_BR_DIR}/dl/gcc-linaro-4.9.4-2017.01-x86_64_arm-linux-gnueabihf.tar.bz2 \
            -C ${tooldir}
...
TARGET_CFLAGS="-pipe -Os -mtune=cortex-a7 -march=armv7-a -mabi=aapcs-linux -mhard-float -D_LARGEFILE_SOURCE -D_LARGEFILE64_SOURCE -D_FILE_OFFSET_BITS=64 -I${INCDIR}"
