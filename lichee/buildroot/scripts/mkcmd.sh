...
mk_info "build kernel ..."
	mk_info "${LICHEE_PLATFORM}"
	mk_info "${LICHEE_KERN_DIR}/.config.mark"
	mk_info "${LICHEE_PLATFORM} ${config_mark}"
	mk_info "(cd ${LICHEE_KERN_DIR} && [ -x ${build_script} ] && ./${build_script})"

    #local build_script="scripts/build_${LICHEE_BOARD}.sh"
    local build_script="scripts/build.sh"
...
function pack_ext4fs()
{
    rm -rf rootfs_oasis
    mk_info "generating rootfs..."

    NR_SIZE=`du -sm ${LICHEE_BR_OUT}/target/ | awk '{print $1}'`
    NEW_NR_SIZE=$(((($NR_SIZE+32)/16)*16))
    #NEW_NR_SIZE=360
    TARGET_IMAGE=$LICHEE_PLAT_OUT/rootfs.ext4

    echo "blocks: $NR_SIZE"M" -> $NEW_NR_SIZE"M""
    echo "BR_DIR:" $LICHEE_BR_DIR
    echo "PLAT_OUT:" $LICHEE_PLAT_OUT
    $LICHEE_PLAT_OUT/buildroot/host/usr/bin/make_ext4fs -s -l $NEW_NR_SIZE"M" $TARGET_IMAGE ${LICHEE_BR_OUT}/target/
...
function pack_oasys()
{
	TARGET_FS=${LICHEE_BR_DIR}/target/${LICHEE_BOARD}/system
	TARGET_IMAGE=$LICHEE_PLAT_OUT/system.ext4

	if [ -d ${TARGET_FS} ]; then
		mk_info "generating system fs..."

		NR_SIZE=`du -sm ${TARGET_FS} | awk '{print $1}'`
		NEW_NR_SIZE=$((((($NR_SIZE+15)/16)*16)+10))
		NEW_NR_SIZE=42

		echo "blocks: $NR_SIZE"M" -> $NEW_NR_SIZE"M""
		echo "BR_DIR:" $LICHEE_BR_DIR
		echo "PLAT_OUT:" $LICHEE_PLAT_OUT
		$LICHEE_PLAT_OUT/buildroot/host/usr/bin/make_ext4fs -b 1024 -s -l $NEW_NR_SIZE"M" $TARGET_IMAGE ${TARGET_FS}
		fsck.ext4 -y $TARGET_IMAGE > /dev/null
		echo "success in generating system fs"
...
function mkpack()
{
	...
	if [ ${LICHEE_BOARD} = "oasis_nand" ] || [ ${LICHEE_BOARD} = "mav3_nand" ] ; then
		LICHEE_MEDIA=spinand
	else
		LICHEE_MEDIA=spinor
	fi

    (cd ${LICHEE_TOOLS_DIR}/pack && \
    	./pack -c ${LICHEE_CHIP} -p ${LICHEE_PLATFORM} -b ${LICHEE_BOARD} -e ${LICHEE_MEDIA} $@)
}
