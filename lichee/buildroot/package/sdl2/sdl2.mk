...
SDL2_CONF_OPT += --enable-video-x11 \
	--with-x=$(STAGING_DIR) \
...
SDL2_CONF_OPT += --enable-video-x11-xcursor
else
SDL2_CONF_OPT += --disable-video-x11-xcursor
endif

ifeq ($(BR2_PACKAGE_XLIB_LIBXINERAMA),y)
SDL2_DEPENDENCIES += xlib_libXinerama
SDL2_CONF_OPT += --enable-video-x11-xinerama
else
SDL2_CONF_OPT += --disable-video-x11-xinerama
endif

ifeq ($(BR2_PACKAGE_XLIB_LIBXI),y)
SDL2_DEPENDENCIES += xlib_libXi
SDL2_CONF_OPT += --enable-video-x11-xinput
else
SDL2_CONF_OPT += --disable-video-x11-xinput
endif

ifeq ($(BR2_PACKAGE_XLIB_LIBXRANDR),y)
SDL2_DEPENDENCIES += xlib_libXrandr
SDL2_CONF_OPT += --enable-video-x11-xrandr
else
SDL2_CONF_OPT += --disable-video-x11-xrandr
endif

ifeq ($(BR2_PACKAGE_XLIB_LIBXSCRNSAVER),y)
SDL2_DEPENDENCIES += xlib_libXScrnSaver
SDL2_CONF_OPT += --enable-video-x11-scrnsaver
else
SDL2_CONF_OPT += --disable-video-x11-scrnsaver
endif

ifeq ($(BR2_PACKAGE_XLIB_LIBXXF86VM),y)
SDL2_DEPENDENCIES += xlib_libXxf86vm
SDL2_CONF_OPT += --enable-video-x11-vm
else
SDL2_CONF_OPT += --disable-video-x11-vm
endif

else
SDL2_CONF_OPT += --disable-video-x11 --without-x
endif

ifeq ($(BR2_PACKAGE_SDL2_OPENGL),y)
SDL2_CONF_OPT += --enable-video-opengl
SDL2_DEPENDENCIES += libgl
else
SDL2_CONF_OPT += --disable-video-opengl
endif
...
SDL2_CONF_OPT += --enable-alsa
else
SDL2_CONF_OPT += --disable-alsa
endif

$(eval $(call AUTOTARGETS,package,sdl2))
