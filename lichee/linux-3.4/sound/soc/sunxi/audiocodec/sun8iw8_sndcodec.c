...
static  bool mic1_mic3_used    = false;
...
static void get_audio_param(void)
{
	...
	if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
        ...
        type = script_get_item("audio0", "mic1_mic3_used",&val);
        if(SCIRPT_ITEM_VALUE_TYPE_INT != type){
                pr_err("[audiocodec] mic1_mic3_used type err! \n");
        }else{
                mic1_mic3_used = val.val;
        }
	...
	struct snd_soc_dai *dai)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		codec_wr_prcm_control(ZERO_CROSS_CTRL, 0x1, ZERO_CROSS_EN, 0x0);
		pa_vol = (read_prcm_wvalue(LINEOUT_VOLC) >> LINEOUTVOL) & 0x1f;
