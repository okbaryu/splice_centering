...
unsigned long normal_standby_autotest = 0;
...
static int aw_pm_begin(suspend_state_t state)
{
	...
	PM_DBG("version 2014�?9�?5�?星期�?09�?4�?4秒_2b7dd9f33b4db4f767f10ac3e26343b3ed04ce04 %d state begin\n", state);
	//set freq max
#ifdef CONFIG_CPU_FREQ_USR_EVNT_NOTIFY
...
#define PM_NAME_LEN (25)
	...
	printk(KERN_INFO "pmu%d_enable = %x\n", num, pmux_enable);
...
static int config_dynamic_standby(void)
{
	...
	printk(KERN_INFO "dynamic_standby = %d.\n", enable);
    if(1 == enable){
...
module_param_named(normal_standby_autotest, normal_standby_autotest, ulong, S_IRUGO | S_IWUSR);
