...
static int axp_gpio_get_data(int gpio)
{
	...
	switch(gpio){
		...
		case 2:
#ifndef CONFIG_AW_AXP20	
			printk("This IO is not an input,no return value!");return -ENXIO;
#else
			axp_read(axp->dev,AXP_GPIO01_STATE,&ret);ret &= 0x40;ret = ret>>6;break;
#endif
...
#else
				axp_clr_bits(axp->dev,AXP_GPIO2_CFG,0x7);
				return axp_set_bits(axp->dev,AXP_GPIO2_CFG,0x01);
#endif
			case 3: 
...
#else
				return axp_clr_bits(axp->dev,AXP_GPIO2_CFG,0x07);
#endif
			case 3: 
	...
	if(mux == 1){//output
		...
		switch(gpio){
			...
			case 2: 
					...
					return -EINVAL;
#else
					axp_clr_bits(axp->dev,AXP_GPIO1_CFG,0x05);
...
static int axp_pmx_get(int gpio)
{
	...
	case 2:
			...
			return 1;
#else
			axp_read(axp->dev, AXP_GPIO2_CFG, &data);
			if (0 == (data & 0x06))
				return 1;
			else if (0x02 == (data & 0x07))
				return 0;
