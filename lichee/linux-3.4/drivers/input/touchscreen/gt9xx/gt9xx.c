...
static void gtp_touch_down(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
    ...
    //    printk("source data:ID:%d, X:%d, Y:%d, W:%d\n", id, x, y, w);
        ...
        //printk("report data:ID:%d, X:%d, Y:%d, W:%d %d %d \n", id, x, y, w, SCREEN_MAX_X, SCREEN_MAX_Y);
...
static void gtp_touch_up(struct goodix_ts_data* ts,s32 id,s32 x,s32 y,s32 w)
{
      ...
      // printk( "source data1:ID:%d, X:%d, Y:%d, W:%d\n", id, x, y, w);
        dprintk(DEBUG_X_Y_INFO,"report data:ID:%d, X:%d, Y:%d, W:%d\n", id, x, y, w);
    	input_report_abs(ts->input_dev, ABS_X, x);
    	input_report_abs(ts->input_dev, ABS_Y, y);
...
static void gtp_touch_up(struct goodix_ts_data* ts, s32 id)
{
	...
	// printk("report data gtp_touch_up");
#if     GTP_ICS_SLOT_REPORT
        input_mt_slot(ts->input_dev, id);
...
*******************************************************/
	static s32 gtp_init_panel(struct goodix_ts_data *ts)
	{
		...
		{
            index = 1;
        }else if(screen_max_x == 480 && screen_max_y == 272){
            index = 0;
		}else{
            index = 1;
		}
...

		{
            int i = 0;
			for(i = 0; i < 10; i++){
    			printk(" [%d] = %x   ", i, send_cfg_buf[index][i]);
			}
            printk("\n");
		}
