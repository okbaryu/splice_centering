...
inline void *__map_buf_kernel_cache(unsigned int phys_addr, unsigned int size)
{
	int npages = PAGE_ALIGN(size) / PAGE_SIZE;
	struct page **pages = vmalloc(sizeof(struct page *) * npages);
	struct page **tmp = pages;
...
void *sunxi_buf_alloc_cache(unsigned int size, unsigned int *paddr)
{
	void *vaddr = NULL;
	...
	*paddr = __carveout_alloc(size);
	...
	vaddr = __map_buf_kernel_cache(*paddr, size);
	if (!vaddr) {
		pr_err("%s(%d) err: map kernel failed!\n", __func__, __LINE__);
		return NULL;
	}
...
EXPORT_SYMBOL(sunxi_buf_alloc_cache);
