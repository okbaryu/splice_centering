...
static int sunxi_fb_ioctl(struct fb_info *info, unsigned int cmd,unsigned long arg)
{
	...
	{
		//printk("###dev fb FBIO_CACHE_SYNC\n");
		if (copy_from_user((void*)karg,(void __user*)arg,2*sizeof(unsigned long))) {
			__wrn("copy_from_user fail\n");
			return -EFAULT;
		}
		//printk("karg[0x%lx,0x%lx]\n", karg[0], karg[1]);
		flush_user_range((long)karg[0],(long)karg[1]);
		break;
	}
	default:
		break;
...
static s32 display_fb_request(u32 fb_id, disp_fb_create_info *fb_para)
{
	...
	for(sel = 0; sel < num_screens; sel++) {
		if(sel == fb_para->fb_mode)	{
			...
			// FIXME: Loading landscape logo for portrait LCD causes kernel hang on boot.
			...
			if(mgr && mgr->rot_sw &&
				...
				config.info.screen_win.width = (0 == fb_para->output_height) ? src_height : fb_para->output_height;
...
s32 fb_init(struct platform_device *pdev)
{
	...
	for(i=0; i<8; i++) {
		...
		g_fbi.fbinfo[i]->var.xres            = 480;
		g_fbi.fbinfo[i]->var.yres            = 272;
		g_fbi.fbinfo[i]->var.xres_virtual    = 480;
		g_fbi.fbinfo[i]->var.yres_virtual    = 272*2;
		...
		{
			__inf("[ERROR]Failed to fb_alloc_cmap(&g_fbi.fbinfo[%d]->cmap, 256, 1)\n", i);
			return -ENOMEM;
		}
	...
	if(g_disp_drv.disp_init.b_init) {
		...
		for(i = 0; i<fb_num; i++)	{
			...
			screen_width = bsp_disp_get_screen_width_from_output_type(screen_id,
								g_disp_drv.disp_init.output_type[screen_id], g_disp_drv.disp_init.output_mode[screen_id]);
			screen_height = bsp_disp_get_screen_height_from_output_type(screen_id,
								g_disp_drv.disp_init.output_type[screen_id], g_disp_drv.disp_init.output_mode[screen_id]);
			fb_para.output_width = screen_width;
			fb_para.output_height = screen_height;

			mgr = disp_get_layer_manager(screen_id);
			if (mgr->device->get_rotation_degree) {
				switch (mgr->device->get_rotation_degree(mgr->device)) {
					case 90:
					case 270:
						fb_para.output_height = screen_width;
						fb_para.output_width = screen_height;
						break;
...
//#endif
	...
	{
		__inf("[ERROR]Failed to (g_disp_drv.disp_init.b_init == false)\n");
