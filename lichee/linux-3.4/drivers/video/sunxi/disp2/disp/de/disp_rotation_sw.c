...
extern void *disp_malloc(u32 num_bytes,u32 * phys_addr); //uncache
extern void disp_free(void *virt_addr, void* phy_addr, u32 num_bytes);
extern void *Fb_map_kernel(unsigned long phys_addr,unsigned long size); //uncache
...
void *disp_malloc_cache(u32 num_bytes, u32 *phys_addr)
{
	...
	if(num_bytes != 0) {
		...
		address = sunxi_buf_alloc_cache(actual_bytes, phys_addr);
		if (address) {
			//__inf("disp_malloc_cache ok, address=0x%x, size=0x%x\n", *phys_addr, num_bytes);
			return address;
		} else {
			DE_WRN("disp_malloc_cache fail, size=0x%x\n", num_bytes);
			return NULL;
		}
	} else {
	...
	disp_layer_config *lyr_config, disp_rect src_dirty_rect)
{
	...
	case ROTATION_SW_270:
		break;
	case ROTATION_SW_0:
		//printk("###ROTATION_SW_0: chn=%d,lyr_id=%d###\n",
		//	lyr_config->channel, lyr_config->layer_id);
		goto out;
	default:
