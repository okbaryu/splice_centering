...
static s32 lcd_parse_panel_para(u32 disp, disp_panel_para * info)
{
    ...
    ret = disp_sys_script_get_item(primary_key, "lcd_model_name", (int*)info->lcd_model_name, 25/sizeof(int));
           ...
           printk("[DISP][disp_lcd] : %s called  disp(screen)id : %d  w:%d, x:%  \n");
    return 0;
}
...
//backlight adjust
	...
	ret = disp_sys_script_get_item(primary_key, "lcd_rotate", &value, 1);
	if (ret < 0) {
		lcd_cfg->rotate = false;
	} else {
		lcd_cfg->rotate = value;
	}

	ret = disp_sys_script_get_item(primary_key, "lcd_rotation_degree", &value, 1);
	if (ret < 0) {
		lcd_cfg->rotation_degree = 0;
	} else {
		lcd_cfg->rotation_degree = value;
	}
...
static int lcd_reinit_timeout = 0;
extern void lcd_reflush(void);
...
#endif
{
	...
	if(disp_al_lcd_query_irq(disp, LCD_IRQ_TCON0_CNTR, &lcdp->panel_info)
		...
		if (0 == lcdp->tri_finish_fail) {
			...
			/*
			 * FIXME:
			 * lcd_reflush(): just for lcd ili9341
			 * delete it when change to another lcd.
			 */
			if (lcd_reinit) {
				if (lcd_reinit_timeout >= 120) {
					lcd_reinit_timeout = 0;
					lcd_reflush();
				} else {
					lcd_reinit_timeout++;
				}
...
static int disp_lcd_get_rotation_degree(struct disp_device* lcd)
{
	struct disp_lcd_private_data *lcdp = disp_lcd_get_priv(lcd);
	...
	if (lcdp->lcd_cfg.rotate) {
		return lcdp->lcd_cfg.rotation_degree;
	}
...
s32 disp_init_lcd(disp_bsp_init_para * para)
{
	...
	for(disp=0; disp<num_screens; disp++) {
		...
		lcd->get_rotation_degree = disp_lcd_get_rotation_degree;
