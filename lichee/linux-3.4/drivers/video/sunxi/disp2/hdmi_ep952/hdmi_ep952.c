...
static disp_gpio_set_t powen_gpio[1];
static u32 powen_gpio_used = 0;
static int ep952_hdl = 0;
...
{
	if(powen_gpio_used == 0 && hdmi_power_used == 0)
		return 0;

#if 1	//powen_gpio_used = 1
	if(on_off)
        __gpio_set_value(ep952_hdl, 0);
    else
        __gpio_set_value(ep952_hdl, 1);
#else	// hdmi_power_used =1
	if(on_off)
...
static s32 ep952_close(void)
{
	...
	disp_sys_gpio_release(ep952_hdl, 2);
    msleep(100);
    return 0;
}
...
*/
    ...
    if(1 == ret) {
       ...
       if(hdmi_used) {
        	...
        	sprintf(io_name,"%s","hdmi_powen");
			printk("%s:%d####io_name = %s ########into\n",__func__,__LINE__,io_name);
        	ret = disp_sys_script_get_item(key_name, io_name, (int *)powen_gpio, sizeof(disp_gpio_set_t)/sizeof(int));
        	if(ret == 3) {
            	powen_gpio_used = 1;			
				ep952_hdl = disp_sys_gpio_request(powen_gpio, 1);
				ret = gpio_direction_output(ep952_hdl, 0);
//				__gpio_set_value(ep952_hdl, 1);
//				gpio_free(ep952_hdl);
        	}
#else
