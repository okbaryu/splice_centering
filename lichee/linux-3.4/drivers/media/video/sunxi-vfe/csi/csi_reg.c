...
{
	//csi_if_cfg(off 0x4): vip1 interface 4 src_type 0 data_width 4
	vfe_dbg(0, "%s(off 0x%x): vip%d interface %d src_type %d data_width %d\n", __func__, CSI_IF_CFG_REG_OFF, sel, csi_if_cfg->interface, csi_if_cfg->src_type, csi_if_cfg->data_width);
...
{
	vfe_dbg(0, "%s(off 0x%x): vip%d  vref %d href %d sample %d (CLK_POL %d), field %d\n", __func__, CSI_IF_CFG_REG_OFF, sel, csi_tmg_cfg->vref, csi_tmg_cfg->href, csi_tmg_cfg->sample, CLK_POL, csi_tmg_cfg->field);
...
{
	//csi_fmt_cfg(off 0x44): vip1 ch[0] input_fmt 3 output_fmt 5 field_sel 2 input_seq 3
	vfe_dbg(0, "%s(off 0x%x): vip%d ch[%d] input_fmt %d output_fmt %d field_sel %d input_seq %d\n", __func__, CSI_CH_CFG_REG_OFF, sel, ch, csi_fmt_cfg->input_fmt, csi_fmt_cfg->output_fmt, csi_fmt_cfg->field_sel, csi_fmt_cfg->input_seq);

#if 0
	if(sel == 1) {
		//csi_fmt_cfg->output_fmt = 2; //field planar yuv420 (YV12, yuv420p)
		//csi_fmt_cfg->output_fmt = 4; //field planar yuv422 uvcombined
		//csi_fmt_cfg->output_fmt = 5; //field planar yuv420 uvcombined (NV21, yuv420sp)
		//csi_fmt_cfg->input_seq = 1; //YVYU
		//csi_fmt_cfg->input_seq = 2; //UYVY
		//csi_fmt_cfg->input_seq = 3; //VYUY
		//when HFLIP set, the board hangs.
		//when VFLIP set, it doesnot hang, but green
		//vfe_reg_clr_set(csi_base_addr[sel] + CSI_CH_CFG_REG_OFF + ch * CSI_CH_OFF, 1<<CSI_CH_CFG_REG_VFLIP_EN, 1<<CSI_CH_CFG_REG_VFLIP_EN);
		//csi_fmt_cfg->field_sel = 0; //capturing with field 1
		//csi_fmt_cfg->field_sel = 1; //capturing with field 2
	}
#endif
...
{
	//csi_set_size(off 0x80): vip0 ch[0] length_h 1920 length_v 1080 buf_length_y 1920 buf_length_c 1920
	vfe_dbg(0, "%s(off 0x%x): vip%d ch[%d] length_h %d length_v %d buf_length_y %d buf_length_c %d\n", __func__, CSI_CH_HSIZE_REG_OFF, sel, ch, length_h, length_v, buf_length_y, buf_length_c);
...
{
	vfe_dbg(0, "%s(off 0x%x): vip%d ch[%d] start_h %d start_v %d\n", __func__, CSI_CH_HSIZE_REG_OFF, sel, ch, start_h, start_v);
