...
//depends on bus interface
    vfe_dbg(0, "%s: vip%d bus_if %d\n", __func__, sel, bus_info->bus_if);
  ...
  vfe_dbg(0, "%s: vip%d ch_total_num %d\n", __func__, sel, bus_info->ch_total_num);
  ...
  //depends on bus format
    ...
    vfe_dbg(0, "%s: vip%d ch[%d] bus_ch_fmt 0x%x bus_pix_type 0x%x bus_width %d bus_precision %d data_width %d ch_field %d pix_ch_fmt 0x%x\n", __func__, sel, ch, bus_info->bus_ch_fmt[ch], bus_pix_type[ch], bus_width[ch], bus_precision[ch], if_cfg.data_width, frame_info->ch_field[ch], frame_info->pix_ch_fmt[ch]);
  ...
  //depends on bus format and target frame format
   ...
   vfe_dbg(0, "%s: vip%d ch[%d] input_fmt %d output_fmt %d input_seq %d\n", __func__, sel, ch, fmt_cfg[ch].input_fmt, fmt_cfg[ch].output_fmt, fmt_cfg[ch].input_seq);
...
//set csi timing parameter
    ...
    vfe_dbg(0, "%s: vip%d timing href %d vref %d sample %d field %d\n", __func__, sel, tmg_cfg.href, tmg_cfg.vref, tmg_cfg.sample, tmg_cfg.field);
...
int bsp_csi_set_size(unsigned int sel, unsigned int isp_in_use, struct bus_info *bus_info, struct frame_info *frame_info)
{
  enum bit_width bus_width[MAX_CH_NUM];
  enum bit_width bus_precision[MAX_CH_NUM];
  unsigned int is_buf_itl[MAX_CH_NUM];
  ...
  frm_arrange_gbl[sel].column = column;
    ...
    vfe_dbg(0, "%s: vip%d row %d col %d\n", __func__, sel, row, column);
  ...
  //depends on field format
    ...
    vfe_dbg(0, "%s: vip%d ch[%d] bus_ch_fmt 0x%x bus_width %d bus_precision %d input_len_h %d input_len_v %d start_h %d start_v %d\n", __func__, sel, ch, bus_info->bus_ch_fmt[ch], bus_width[ch],  bus_precision[ch], input_len_h[ch], input_len_v[ch], start_h[ch], start_v[ch]);
    ...
    switch(frame_info->pix_ch_fmt[ch]) {
      ...
      case PIX_FMT_YVU420MB_8:
        ...
        if(isp_in_use) {
          ...
          buf_height_y_ch[ch] = CSI_ALIGN_16B(frame_info->ch_size[ch].height);
          buf_height_cb_ch[ch] = CSI_ALIGN_16B(buf_height_y_ch[ch] >>1);
        }
#else
        buf_height_y_ch[ch] = frame_info->ch_size[ch].height;
        buf_height_cb_ch[ch] = buf_height_y_ch[ch] >>1;
   ...
   vfe_dbg(0, "%s: vip%d ch[%d] pix_ch_fmt 0x%x line_stride_y_ch %d line_stride_c_ch %d buf_height_y_ch %d buf_height_cb_ch %d buf_height_cr_ch %d\n", __func__, sel, ch, frame_info->pix_ch_fmt[ch], line_stride_y_ch[ch], line_stride_c_ch[ch], buf_height_y_ch[ch], buf_height_cb_ch[ch], buf_height_cr_ch[ch]);
  ...
  for(i=0;i<row;i++) {
    ...
    for(j=0;j<column;j++) {
      ...
      vfe_dbg(0, "%s: vip%d ch %d row %d col %d line_stride_y_row %d line_stride_c_row %d line_stride_y_row_gbl %d line_stride_c_row_gbl %d\n", __func__, sel, ch, row, column, line_stride_y_row[i], line_stride_c_row[i], line_stride_y_row_gbl[sel][i], line_stride_c_row_gbl[sel][i]);
  ...
  vfe_dbg(0, "%s: vip%d frm_byte_size %d\n", __func__, sel, frame_info->frm_byte_size);
