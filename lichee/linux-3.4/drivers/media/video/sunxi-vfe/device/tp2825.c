...
static int init_std = STD_HDA;
#else
static int init_mode = SENSOR_1080P30;
static int init_std = STD_TVI;
#endif
static int chips = 1;
static int output[MAX_CHIPS] =
{
#if USE_BT656
	MUX656_8BIT
#else
...
//define module timing for csi parallel (bt.656 doesn't care this)
...
#define CLK_POL           V4L2_MBUS_PCLK_SAMPLE_FALLING //V4L2_MBUS_PCLK_SAMPLE_RISING//
...
{
	// Start address 0x15, Size = 9B
	/* H delay(9-8)[5:4], Y delay (luma data delay rel. to chroma data delay) [2:0] */
	{ 0x15, 0x03 },
	/* H delay(7-0): distance from internal sync ref. to start of video output: -512~+511 pixels */
	{ 0x16, 0xd3 },
	/* H active horz output length in output pixels#(7-0): for 1080p, 1920d(780h), for 720p, 1280d(500h)  */
...
static int sensor_v1_setup(struct v4l2_subdev *sd)
{
	...
	/* LPF control: EQ3BP Bypass(1), Enabled(0)([7]), lpf Bypass(0), 10Mhz(1), 25Mhz(2) 50MHz(3)[3:2] */
	/* LOG_ERR_RET(sensor_write(sd, 0x2b, 0x58)); */
...
static int sensor_set_video_mode(struct v4l2_subdev *sd, int chip, unsigned char mode, unsigned char ch, unsigned char std)
{
	...
	case SENSOR_1080P30:
		...
		*/
		...
		LOG_ERR_RET(sensor_read(sd, 0x4E, &tmp));
		tmp &= 0xFB; //set SYSCLKMD to 148.5Mhz
		LOG_ERR_RET(sensor_write(sd, 0x4E, tmp));
...
{
	uint8_t tmp, vd1_swap = 0;
	#if CONFIG_TP2825_VD1_BYTE_SWAP
	vd1_swap = 0x10;
	#endif
	/*
		0x4c: Video Sync Output Register
		0x4d: VD Pin Output Control Register
		0x4e: CLKO Output Driver Control Register
	*/
	if ( MUX656_8BIT == output[chip] ) {

		vfe_dev_dbg("sensor_output_setup(BT656, %s)\n", get_cvstd_name(mode));

		/*
			[7]: not inverse data valid output signal polarity(0)
			[6]: not inverse field output signal polarity(0)
			[5]: not inverse V-sync output signal polarity(0)
			[4]: not inverse H-sync output signal polarity(0)
			[3]: disable data valid signal output(0)
			[2]: disable field signal output(0)
			[1]: disable V-sync signal output on VS pin(0)
			[0]: disable H-sync signal output on VS pin(0)
		*/
		LOG_ERR_RET(sensor_write(sd, 0x4C, 0x00));
		/*
			[7]: disable Y-ADC[9-2] output on VD2_7-0 pin(0)
			[6]: disable Y-ADC[9-2] output on VD1_7-0 pin(0)
			[5]: VD2_7-0 pin have normal MSB-LSB bit order(default)(0)
			[4]: VD1_7-0 pin have normal MSB-LSB bit order (Default)(0)
			[3]: YCbCr data output in 8bit mode.Y data output in 16bit mode (0)
			[2]: YCbCr data output in 8bit mode.Y data output in 16bit mode (0)
			[1]: VD2_7-0 pin output enable(1)
			[0]: VD1_7-0 pin output enable(1)
		*/

		LOG_ERR_RET(sensor_write(sd, 0x4D, 0x03|vd1_swap)); //both port enable
		if (SENSOR_720P25V2 == mode || SENSOR_720P30V2 == mode || SENSOR_PAL == mode || SENSOR_NTSC == mode ) {
			/*
				[7]: not inverse Video ADC input clock polarity(0)
				[6]: CKPOL controls final CLKO pin output signal polarity after CLKMD selection: not inversed(0)
					0: VD valid @ CLKO rising edge
					1: VD valid @ CLKO falling edge
				[5:4]: CLKMD controls CLKO2 output. This register selects one of following clocks to be output in CLKO pin.
					0h = 148.5MHz clock
					1h = 74.25MHz clock sync to rising edge of 148.5MHz (*)
					2h = 74.25MHz clock sync to falling edge of 148.5MHz
					3h = VD1120CKO clock. If MD1120=0 16-bit mode is used, this clock must be selected.
				[3]: SYSCLKPD Video Decoder is in normal clock operation (0)
				[2]: SYSCLKMD
					Video Decoder system clock is 148.5MHz (0)
					Video Decoder system clock is 74.25MHz (1) (*)
				[1]: CK12OE Disable 8mA output driver on CLKO pin (Default (0)
				[0]: CK11OE Enable 12mA output driver on CLKO pin (1)
			*/
			LOG_ERR_RET(sensor_write(sd, 0x4E, 0x15));
		} else {
			LOG_ERR_RET(sensor_write(sd, 0x4E, 0x01));
		}
		/* CLKO delay: 0.2ns * value[3:0] */
		//LOG_ERR_RET(sensor_write(sd, 0xf3, 0x00));
	} else if ( SEP656_8BIT == output[chip] ) {

		vfe_dev_dbg("sensor_output_setup(BT601, %s)\n", get_cvstd_name(mode));

		LOG_ERR_RET(sensor_write(sd, 0x4C, 0x03));
		LOG_ERR_RET(sensor_write(sd, 0x4D, 0x03|vd1_swap)); //both port enable
		//LOG_ERR_RET(sensor_write(sd, 0x4D, 0x81));
		if (SENSOR_720P25V2 == mode || SENSOR_720P30V2 == mode || SENSOR_PAL == mode || SENSOR_NTSC == mode ) {
			LOG_ERR_RET(sensor_write(sd, 0x4E, 0x15));
		} else {
			LOG_ERR_RET(sensor_write(sd, 0x4E, 0x01));
		...
		LOG_ERR_RET(sensor_write(sd, 0x4C, 0x00));
		LOG_ERR_RET(sensor_write(sd, 0x4D, 0x0B|vd1_swap));
		if (SENSOR_720P25V2 == mode || SENSOR_720P30V2 == mode || SENSOR_PAL == mode || SENSOR_NTSC == mode ) {
...
static int sensor_comm_init(struct v4l2_subdev *sd, int chip)
{
	...
	LOG_ERR_RET(sensor_write(sd, 0x3b, 0x33)); // ADC reset
	LOG_ERR_RET(sensor_write(sd, 0x3b, 0x03));
...
//  vfe_dev_dbg("[]arg=%0x\n",arg);
	...
	case GET_CURRENT_WIN_CFG:
		vfe_dev_dbg("ioctl(GET_CURRENT_WIN_CFG) = %p\n", info->current_wins);
		if (info->current_wins != NULL) {
			vfe_dev_dbg("current wsize %dx%d\n", info->current_wins->width, info->current_wins->height);
...
static struct sensor_win_size sensor_win_sizes[] = {
	...
	*/
	...
	/*
		8-bit bus, YUV422, 1080p@30
		2200x1125x2x30 = 148.5MHz
	*/
	/* 1080p */
	{
		.width		= HD1080_WIDTH,
                                   ...
                                   struct sensor_win_size **ret_wsize)
{
	...
	vfe_dev_dbg("sensor_try_fmt_internal(bus code 0x%x, %dx%d)\n", fmt->code, fmt->width, fmt->height);
	//V4L2_PIX_FMT_YVU420
                        ...
                        struct v4l2_mbus_framefmt *fmt)
{
	...
	vfe_dev_dbg("sensor_s_fmt(capture mode %d, mbus %x (V4L2_MBUS_FMT_UYVY8_2X8=%x), %dx%d)\n", info->capture_mode, fmt->code, V4L2_MBUS_FMT_UYVY8_2X8, fmt->width, fmt->height);

//  sensor_write_array(sd, sensor_oe_disable_regs, ARRAY_SIZE(sensor_oe_disable_regs));

	ret = sensor_try_fmt_internal(sd, fmt, &sensor_fmt, &wsize);
	vfe_dev_dbg("==> sensor_try_fmt_internal() ret = %d, wsize=%dx%d\n", ret, ret == 0 ? wsize->width : 0, ret == 0 ? wsize->height : 0);
...
static int sensor_s_parm(struct v4l2_subdev *sd, struct v4l2_streamparm *parms)
{
	...
	vfe_dev_dbg("sensor_s_parm(type %d, numerator %d, denominator %d)\n", parms->type, tpf->numerator, tpf->denominator);
...
{
#if ENV_HDA
	vfe_dev_dbg("init_sensor: HDA mode\n");
#else
	vfe_dev_dbg("init_sensor: TVI mode\n");
#endif
...
#if ENV_HDA
			chip_info[i].scan[j] = SCAN_HDA;
			chip_info[i].std[j] = STD_HDA;
#else
