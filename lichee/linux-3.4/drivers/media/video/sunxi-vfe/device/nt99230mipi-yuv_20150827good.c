/*
 * A V4L2 driver for nt99230_mipi cameras.
 * Novatek 2013-12-10
 */
...
MODULE_DESCRIPTION("A low-level driver for nt99230_mipi sensors");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN      0 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) printk("[NT99230_mipi]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) printk("[CSI_ERR][nt99230_mipi]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[CSI][nt99230_mipi]"x,##arg)
...
/*
 * The nt99230_mipi sits on i2c with ID 0x66
 */
#define I2C_ADDR 0x66
#define NT99231_WRITE_ADDR (0x66)
#define NT99231_READ_ADDR  (0x67)
#define SENSOR_NAME "nt99230_mipi"


static struct v4l2_subdev *nt99230_mipi_sd;
...
static struct regval_list sensor_1080p_raw_regs[] = { //1080: 1920*1080@30fps
#if 1
	{0x3012, 0x03}, 
	{0x3013, 0xE4},
	{0x3069, 0x03}, 
...
static struct regval_list sensor_1080p_yuv_regs[] = { //1080: 1920*1080@30fps
#if 1
	{0x3012, 0x03}, 
	{0x3013, 0xE4},
	{0x3069, 0x03}, 
	...
	{0x3010, 0x04},                       
	{0x3011, 0x38},  //Timing End         
	{0x3508, 0x01},  //MIPI Start         
	{0x3509, 0x7B},                       
	{0x3500, 0xE4},  //MIPI End      
	{0x3514, 0x01},  //MIPI End      
	{0x320A, 0x00},                       
	{0x3021, 0x02},                       
	{0x3060, 0x01},                       
	{0x301e, 0x08}, 					  
...
static struct regval_list sensor_720p_regs[] = { //720: 1280*720@30fps
	...
	{0x32EA, 0xAA}, 
	{0x32EB, 0xAA}, 
	{0x32EC, 0xAA},  //Scale End
	{0x32F0, 0x00},  //Output Format
	{0x3200, 0x3E},  //Mode Control
	{0x3201, 0x7F},  //Mode Control
	{0x302A, 0x80},  //PLL Start
	{0x302B, 0x00}, 
	{0x302C, 0x31}, 
	{0x302D, 0x08}, 
...
static int sensor_s_exp_gain(struct v4l2_subdev *sd, struct sensor_exp_gain *exp_gain)
{
  ...
  shutter = exp_val/16;
  if(shutter > nt99230_sensor_vts- 4)
		frame_length = shutter + 4;
  else
		frame_length = nt99230_sensor_vts;
...
static int sensor_detect(struct v4l2_subdev *sd)
{
	...
	{
		printk(KERN_DEBUG"*********find nt99231 raw/yuv data camera sensor now.\n");
		return 0;
	}
}
...
/*	{
		.desc		= "Raw RGB Bayer",
		.mbus_code	= V4L2_MBUS_FMT_SRGGB10_10X1,//V4L2_MBUS_FMT_SGRBG10_10X1,
		.regs 		= sensor_fmt_raw,
		.regs_size = ARRAY_SIZE(sensor_fmt_raw),
		.bpp		= 1
	},
	{
		...
		.regs 		= sensor_fmt_mipi_rgb8,
		.regs_size = ARRAY_SIZE(sensor_fmt_mipi_rgb8),
		.bpp		= 1
	},*/
};
...
/*	{
		...
		.gain_min   = 1<<4,
		.gain_max   = 16<<4,
		.regs       = sensor_1080p_raw_regs,//
		.regs_size  = ARRAY_SIZE(sensor_1080p_raw_regs),//
		.set_size	= NULL,
	},*/
	/* 720P MIPI RAW*/
/*	{
	...
	/* 1080P MIPI YUV*/
	{
		...
		.gain_min   = 1<<4,
		.gain_max   = 16<<4,
		.regs       = sensor_1080p_yuv_regs,//
		.regs_size  = ARRAY_SIZE(sensor_1080p_yuv_regs),//
		.set_size	= NULL,
	},
	/* 720P MIPI YUV*/
	{
