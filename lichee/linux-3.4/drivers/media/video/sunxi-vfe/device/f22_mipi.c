/*
 * A V4L2 driver for F22_mipi Raw cameras.
 *
 */
...
MODULE_AUTHOR("lwj");
MODULE_DESCRIPTION("A low-level driver for F22_mipi Raw sensors");
MODULE_LICENSE("GPL");

//for internel driver debug
#define DEV_DBG_EN      1 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) printk("[F22_mipi Raw]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) printk("[F22_mipi Raw]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[F22_mipi Raw]"x,##arg)
...
#define V4L2_IDENT_SENSOR  0x0F22
...
/*
 * The f22_mipi i2c address
 */
#define F22_WRITE_ADDR (0x80)
#define F22_READ_ADDR  (0x81)

//static struct delayed_work sensor_s_ae_ratio_work;
static struct v4l2_subdev *glb_sd;
#define SENSOR_NAME "f22_mipi"
...
//	printk("f22_mipi sensor_set_exp = %d Done!\n", exp_val );
...
//  printk("f22 sensor exp & gain is %d\t%d\n", exp_val, gain_val);
...
static int sensor_detect(struct v4l2_subdev *sd)
{
  ...
  vfe_dev_dbg("f22_mipi ID_VAL_HIGH = %2x, Done!\n", rdval);
  if(rdval != ID_VAL_HIGH)
    return -ENODEV;
 

  LOG_ERR_RET(sensor_read(sd, ID_REG_LOW, &rdval))
  vfe_dev_dbg("f22_mipi ID_VAL_LOW = %2x, Done!\n", rdval);
  if(rdval != ID_VAL_LOW)
    return -ENODEV;
