...
#define DEV_DBG_EN      1 
#if(DEV_DBG_EN == 1)    
#define vfe_dev_dbg(x,arg...) 	printk("[OV683]"x,##arg)
#else
#define vfe_dev_dbg(x,arg...) 
#endif
#define vfe_dev_err(x,arg...) 	printk("[OV683]"x,##arg)
#define vfe_dev_print(x,arg...) printk("[OV683]"x,##arg)
...
//#define  DEBUG_OV683 			1
//define module timing

#define MCLK              (24*1000*1000)      // (12*1000*1000)
...
{	int i =0;
	...
	{		if((array+i)->addr == REG_DLY) 
		...
		printk("ov683 0x%x value is 0x%x\n",(array+i)->addr,tmp);
	}
	return 0;
}
*/
...
static int reg_val_store(struct v4l2_subdev *sd, unsigned short reg,
    unsigned char value)
{
	printk("write 0x%x value is 0x%x\n",reg,value);
...
//		while((reg_val_show(sd,0x150e) != 0x88)&&(i++ < 10)) {	
//			msleep(10);
//		}
	} else {
		printk("stream_control off :\n");
...
static struct sensor_format_struct {
  ...
  int bpp;   /* Bytes per pixel */
	...
	{
		 .desc   = "MIPI YUV422",
    .mbus_code  = V4L2_MBUS_FMT_YUYV8_16X1,// V4L2_MBUS_FMT_YUYV8_16X1,
...
#if DEBUG_OV683
	reg_val_show(sd,0x2080);    // 0xc0
	reg_val_show(sd,0x3080);	// 0xc0
	reg_val_show(sd,0x6638);	
	reg_val_show(sd,0x6639);	
	reg_val_show(sd,0x663a);	
	reg_val_show(sd,0x663b);	
	reg_val_show(sd,0x6738);	
	reg_val_show(sd,0x6739);	
	reg_val_show(sd,0x673a);	
	reg_val_show(sd,0x673b);	
	reg_val_show(sd,0x76c0);	
	reg_val_show(sd,0x76c1);

	reg_val_show(sd,0x665d);   //value: 0x32 
	reg_val_show(sd,0x1300);  //46
    reg_val_show(sd,0x1301);  //88
	reg_val_show(sd,0x1302);  //46
	reg_val_show(sd,0x1303);  //88
