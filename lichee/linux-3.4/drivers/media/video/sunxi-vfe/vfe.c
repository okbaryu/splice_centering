...
#ifdef CONFIG_ARCH_SUN8IW8P1
/* by sjjnn - V4L2_PIX_FMT_RGBXXXX - kernel panic.
...
#endif
		vfe_dbg(2, "combinate shutter = %d, gain =%d \n", dev->isp_3a_result_pt->exp_line_num,
...
static irqreturn_t vfe_isr(int irq, void *priv)
{
	...
	/*
	vfe_print("vfe isr(vip%d ch%d isp%d raw%d: cd%d fd%d b0ovf%d b1ovf%d b2ovf%d pe%d hovf%d vt%d)\n", dev->vip_sel, dev->cur_ch, dev->is_isp_used, dev->is_bayer_raw,
		status.capture_done, status.frame_done, status.buf_0_overflow,
		status.buf_1_overflow, status.buf_2_overflow, status.protection_error,
		status.hblank_overflow, status.vsync_trig);
	*/

	if ( (status.capture_done == 0) && (status.frame_done == 0) && (status.vsync_trig == 0) )
	{
		/*
		vfe_print("enter vfe int for nothing(vip%d ch%d isp%d raw%d: cd%d fd%d b0ovf%d b1ovf%d b2ovf%d pe%d hovf%d vt%d)\n", dev->vip_sel, dev->cur_ch, dev->is_isp_used, dev->is_bayer_raw,
			status.capture_done, status.frame_done, status.buf_0_overflow,
			status.buf_1_overflow, status.buf_2_overflow, status.protection_error,
			status.hblank_overflow, status.vsync_trig);
		*/
		vfe_print("enter vfe int for nothing(vip%d ch%d isp%d raw%d)\n", dev->vip_sel, dev->cur_ch, dev->is_isp_used, dev->is_bayer_raw);
		...
		/*
		bsp_csi_int_get_status(dev->vip_sel, dev->cur_ch, &status);
		vfe_print("leave vfe int for nothing(vip%d ch%d isp%d raw%d: cd%d fd%d b0ovf%d b1ovf%d b2ovf%d pe%d hovf%d vt%d)\n", dev->vip_sel, dev->cur_ch, dev->is_isp_used, dev->is_bayer_raw,
			status.capture_done, status.frame_done, status.buf_0_overflow,
			status.buf_1_overflow, status.buf_2_overflow, status.protection_error,
			status.hblank_overflow, status.vsync_trig);
		*/
...
static enum v4l2_mbus_pixelcode *try_fmt_internal(struct vfe_dev *dev, struct v4l2_format *f)
{
	...
	vfe_dbg(0, "vip%d: try_fmt_internal\n", dev->vip_sel);
...
//  enum pixel_fmt isp_fmt[ISP_MAX_CH_NUM];
	...
	vfe_dbg(0, "bus pixel code = %x pixel format = %x at %s\n", *bus_pix_code, f->fmt.pix.pixelformat, __func__);
	vfe_dbg(0, "pix->width = %d at %s\n", f->fmt.pix.width, __func__);
	vfe_dbg(0, "pix->height = %d at %s\n", f->fmt.pix.height, __func__);
	...
	if (mbus_cfg.type == V4L2_MBUS_CSI2) {
		...
		vfe_print("vip%d V4L2_MBUS_CSI2,%d lane,bus%d channel,rx %d channel\n", dev->vip_sel, dev->mipi_para.lane_num, dev->total_bus_ch, dev->total_rx_ch);
	} else if (mbus_cfg.type == V4L2_MBUS_PARALLEL) {
		vfe_print("vip%d V4L2_MBUS_PARALLEL\n", dev->vip_sel);
		...
		vfe_print("vip%d V4L2_MBUS_BT656\n", dev->vip_sel);
...
//      printk("dev->mipi_fmt.packet_fmt[%d] = %x\n",i,dev->mipi_fmt.packet_fmt[i]);
	...
	ret = bsp_csi_set_size(dev->vip_sel, dev->is_isp_used, &dev->bus_info, &dev->frame_info);
	if (ret < 0) {
	...
	{
		vfe_dbg(0, "vip%d isp_module_init start!\n", dev->vip_sel);
		...
		vfe_dbg(0, "vip%d isp_module_init end!\n", dev->vip_sel);
	}
	ret = 0;
out:
...
static int internal_s_input(struct vfe_dev *dev, unsigned int i)
{
	...
	if (dev->is_isp_used) {
		...
		if (ret != 0) {
			vfe_err("vip%d sensor sensor_s_ctrl V4L2_CID_VFLIP error %d when vidioc_s_input!input_num = %d,\n", dev->vip_sel, ret, i);
		}
		...
		ret = v4l2_subdev_call(dev->sd, core, s_ctrl, &ctrl);
		if (ret != 0) {
			vfe_err("vip%d sensor sensor_s_ctrl V4L2_CID_HFLIP error %d when vidioc_s_input!input_num = %d\n", dev->vip_sel, ret, i);
		}
	}
...
static int vfe_request_gpio(struct vfe_dev *dev)
{
	...
	vfe_dbg(0, "vip%d vfe_request_gpio\n", dev->vip_sel);
...
static int vfe_gpio_config(struct vfe_dev *dev, int bon)
{
	...
	vfe_dbg(0, "vip%d vfe_gpio_config(on %d)\n", dev->vip_sel, bon);
...
static void vfe_gpio_release(struct vfe_dev *dev)
{
	...
	vfe_dbg(0, "vip%d vfe_gpio_release\n", dev->vip_sel);
...
static int vfe_set_sensor_power_on(struct vfe_dev *dev)
{
	...
	usleep_range(10000, 12000);
#endif
vfe_print("[JCNET] in vfe_set_sensor_power_on \n");
...
static void probe_work_handle(struct work_struct *work)
{
	...
	for (input_num = 0; input_num < dev->dev_qty; input_num++)
	{
		...
		{
			if (read_ini_info(dev, input_num, ReparseIniPath))
			{
...
snesor_register_end:
	...
	*vfd = vfe_template[dev->id];
	...
	{
		ret = video_register_device(vfd, VFL_TYPE_GRABBER, -1);//dev->id
		if (ret < 0)
		{
...
static int __init vfe_init(void)
{
	...
	syscfg_get_string("hawkview", "ini_path", ReparseIniPath, sizeof(ReparseIniPath));
	syscfg_get_string("hawkview", "log_path", LogFilePath, sizeof(LogFilePath));
#else
