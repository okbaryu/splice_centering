...
int cfg_get_all_keys_value(char *buffer, struct cfg_mainkey *scts)
{
  ...
  if(scts->cfg_flag != CFG_KEY_INIT)
	...
	if (scts->subkey_cnt > MAX_SUBKEY_NUM) {
		printk(KERN_WARNING "[vfe] key %s contains too many(%d) subkeys.", scts->name, scts->subkey_cnt);
		scts->subkey_cnt = MAX_SUBKEY_NUM;
	}
...
int cfg_read_ini(char *file_path, struct cfg_section **cfg_section)
{
	...
	if (cfg_sct->mainkey_cnt < 0) {
		ret = -1;
		goto rd_ini_end;
	}
	if (cfg_sct->mainkey_cnt > MAX_MAINKEY_NUM) {
		// Probably corrupt INI file loaded.
		printk(KERN_WARNING "[vfe] INI contains too many(%d) keys.", cfg_sct->mainkey_cnt);

		cfg_sct->mainkey_cnt = MAX_MAINKEY_NUM;
	}
