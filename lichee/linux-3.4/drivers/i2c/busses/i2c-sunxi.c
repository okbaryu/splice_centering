	...
	for (i=0; i<SUNXI_TWI_NUM; i++) {
		...
		if (item.val)
			twi_used_mask |= SUNXI_TWI_CHAN_MASK(i);
				/*Mary add bagin*/
				// overlay frequency of twi[i] by appling config
				type = script_get_item(twi_para, "twi_frequency", &item);
				if (SCIRPT_ITEM_VALUE_TYPE_INT != type) {
					I2C_ERR("[twi%d] has no twi_speed!\n", i);
				}else if(item.val>=20000&&item.val<=500000){ //i2c frequency: min 20K; max 500K
					pdata[i].frequency = (unsigned int)item.val;
				    I2C_ERR("[twi%d]:sunxi_i2c_platform_data.frequency == %d ",i,pdata[i].frequency);
				}else{
					I2C_ERR("[twi%d]:i2c:twi_frequency set fail: min 20K; max 500K",i);
				}
				/*Mary add end*/
