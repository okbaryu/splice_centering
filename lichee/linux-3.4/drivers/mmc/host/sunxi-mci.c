...
#if defined(CONFIG_MMC_TRACE_SUNXI) && defined(CONFIG_DEBUG_FS)
#define RECORD_TRACK(m, c)	{	\
	if(m && m->data) {	\
		s32 cur = smc_host->cur_track;	\
		smc_host->cur_track = (smc_host->cur_track+1)%SUNXI_MMC_TRACK_SIZE;	\
		smc_host->tracks[cur].isr = 0;	\
		smc_host->tracks[cur].tag = (s32)c;	\
		smc_host->tracks[cur].id = m->data->host_cookie;	\
		smc_host->tracks[cur].regs[0] = smc_host->tracks[cur].regs[1] = smc_host->tracks[cur].regs[2] = smc_host->tracks[cur].regs[3] = smc_host->tracks[cur].regs[4] = 0; \
		smc_host->tracks[cur].wait = smc_host->wait;	\
		smc_host->tracks[cur].state = smc_host->state;	\
	}	\
}

#define RECORD_TRACK_ISR(m, c, r1, r2, r3, r4, r5)	{	\
	if(m && m->data) {	\
		s32 cur = smc_host->cur_track;	\
		smc_host->cur_track = (smc_host->cur_track+1)%SUNXI_MMC_TRACK_SIZE;	\
		smc_host->tracks[cur].isr = 1;	\
		smc_host->tracks[cur].tag = (s32)c;	\
		smc_host->tracks[cur].id = m->data->host_cookie;	\
		smc_host->tracks[cur].regs[0] = r1; \
		smc_host->tracks[cur].regs[1] = r2;	\
		smc_host->tracks[cur].regs[2] = r3;	\
		smc_host->tracks[cur].regs[3] = r4;	\
		smc_host->tracks[cur].regs[4] = r5; \
		smc_host->tracks[cur].wait = smc_host->wait;	\
		smc_host->tracks[cur].state = smc_host->state;	\
	}	\
}
#else
#define RECORD_TRACK(m, c) do {} while(0);
#define RECORD_TRACK_ISR(m, c, r1, r2, r3, r4, r5) do {} while(0);
#endif

#if defined(CONFIG_MMC_TRACE_SUNXI) && defined(CONFIG_DEBUG_FS)

static int sunxi_mci_trace_show(struct seq_file *s, void *v)
{
	struct sunxi_mmc_host *smc_host = s->private;

	s32 i, cur = smc_host->cur_track;

	seq_printf(s, "cur track#%d\n", cur);

	for(i=0; i<SUNXI_MMC_TRACK_SIZE; i++) {

		if(smc_host->tracks[i].isr) {
			if(i == cur) {
				seq_printf(s, "%3d:%c:%d %08x %08x %08x %08x %08x (%04x %d) <<<\n", i, smc_host->tracks[i].tag, smc_host->tracks[i].id, 
					smc_host->tracks[i].regs[0], smc_host->tracks[i].regs[1], smc_host->tracks[i].regs[2], 
					smc_host->tracks[i].regs[3], smc_host->tracks[i].regs[4], smc_host->tracks[i].wait, smc_host->tracks[i].state);
			} else {
				seq_printf(s, "%3d:%c:%d %08x %08x %08x %08x %08x (%04x %d)\n", i, smc_host->tracks[i].tag, smc_host->tracks[i].id, 
					smc_host->tracks[i].regs[0], smc_host->tracks[i].regs[1], smc_host->tracks[i].regs[2], 
					smc_host->tracks[i].regs[3], smc_host->tracks[i].regs[4], smc_host->tracks[i].wait, smc_host->tracks[i].state);
			...
			if(i == cur) {
				seq_printf(s, "%3d:%c:%d (%04x %d)  <<<\n", i, smc_host->tracks[i].tag, smc_host->tracks[i].id, smc_host->tracks[i].wait, smc_host->tracks[i].state);
			} else {
				seq_printf(s, "%3d:%c:%d (%04x %d)\n", i, smc_host->tracks[i].tag, smc_host->tracks[i].id, smc_host->tracks[i].wait, smc_host->tracks[i].state);	
			}
		}
...
static int sunxi_mci_regs_open(struct inode *inode, struct file *file)
{
	return single_open(file, sunxi_mci_trace_show, inode->i_private);
}

static const struct file_operations sunxi_mci_trace_fops = {
	.owner		= THIS_MODULE,
	.open		= sunxi_mci_regs_open,
	.read		= seq_read,
	.llseek		= seq_lseek,
	.release	= single_release,
};

static void sunxi_mci_init_debugfs(struct sunxi_mmc_host *smc_host)
{
	...
	node = debugfs_create_file("trace", S_IRUSR, root, smc_host,
				   &sunxi_mci_trace_fops);
	if (!node)
		goto err;
...
#endif /* defined(CONFIG_MMC_TRACE_SUNXI) && defined(CONFIG_DEBUG_FS) */
...
exit:
	if (smc_host->pdata->has_pwrsw) {
		SMC_MSG(smc_host, "sdc%d power switch to ON\n", smc_host->pdev->id);
		__gpio_set_value(smc_host->pdata->power.gpio, !smc_host->pdata->power.data);
	}
...
static int card_power_off(struct sunxi_mmc_host* smc_host)
{
	...
	if (smc_host->pdata->has_pwrsw) {
		SMC_MSG(smc_host, "sdc%d power switch to OFF\n", smc_host->pdev->id);
		__gpio_set_value(smc_host->pdata->power.gpio, smc_host->pdata->power.data);
	}
...
static void sunxi_mci_send_cmd(struct sunxi_mmc_host* smc_host, struct mmc_command* cmd)
{
	...
	RECORD_TRACK_ISR(smc_host->mrq, 'C', cmd->opcode, (smc_host->mrq->data ? smc_host->mrq->data->blksz * smc_host->mrq->data->blocks : 0), 0, 0, 0);
	RECORD_TRACK_ISR(smc_host->mrq, 'D', cmd_val, cmd->arg, cmd->flags, cmd->data->flags, imask);
...
static int sunxi_mci_request_mmcio(struct sunxi_mmc_host *smc_host)
{
	...
	if (pdata->has_pwrsw) {
		...
		ret = pin_config_set(SUNXI_PINCTRL, pin_name, config);
		if (ret) {
			SMC_ERR(smc_host, "sdc%d set io(sdc-power) pull failed(%d)\n", smc_no, ret);
			return -1;
		}
		...
		ret = pin_config_set(SUNXI_PINCTRL, pin_name, config);
		if (ret) {
			SMC_ERR(smc_host, "sdc%d set io(sdc-power) drvlel failed\n", smc_no);
			return -1;
		}
		...
		ret = pin_config_set(SUNXI_PINCTRL, pin_name, config);
		if (ret) {
			SMC_ERR(smc_host, "sdc%d set io(sdc-power) mulsel failed\n", smc_no);
			return -1;
		}
	}
...
static irqreturn_t sunxi_mci_irq(int irq, void *dev_id)
{
	...
	RECORD_TRACK_ISR(smc_host->mrq, '!', raw_int, smc_host->int_sum, msk_int, idma_inte, idma_int);
...
static void sunxi_mci_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	if(data) {
		s32 busy = sunxi_mci_check_r1_ready(mmc, 500);
		if(busy) {
			RECORD_TRACK(mrq, '?');
		} else {
			RECORD_TRACK(mrq, '.');
...
#if 0
	ret = sunxi_mci_check_r1_ready(mmc, 500);
	if (ret) {
		mrq->cmd->error = ret;
		mmc_request_done(mmc, mrq);
		return;
...
int sunxi_mci_check_r1_ready(struct mmc_host* mmc, unsigned ms)
{
	...
	if ((mci_readl(smc_host, REG_STAS) & SDXC_CardDataBusy)) {
		...
		dev_err(mmc_dev(mmc), "Error R1 ready %d ms timeout\n", ms);
		return -EIO;
	} else
		return 0;
}
...
#if defined(CONFIG_MMC_TRACE_SUNXI) && defined(CONFIG_DEBUG_FS)
	sunxi_mci_init_debugfs(smc_host);
#endif
...
*/
out:
	if (smc_host && smc_host->pdata->has_pwrsw) {
		SMC_MSG(smc_host, "sdc%d power switch to OFF\n", smc_host->pdev->id);
		__gpio_set_value(smc_host->pdata->power.gpio, smc_host->pdata->power.data);
	}
...
static int sunxi_mci_get_devinfo(void)
{
	...
	for (i=0; i<SUNXI_MCI_DEVNUM; i++) {
		...
		/* get sdc-power information */
		type = script_get_item(mmc_para, "sdc_power", &val);
		if (type != SCIRPT_ITEM_VALUE_TYPE_PIO) {
			mmcinfo->has_pwrsw = 0;
		} else {
			mmcinfo->has_pwrsw = 1;
			mmcinfo->power = val.gpio;
		}
