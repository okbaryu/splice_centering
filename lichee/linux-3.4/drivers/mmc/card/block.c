	...
	struct mmc_ioc_erase_cmd __user *ic_ptr)
{
    ...
    pr_err("\n%s, %d part_num:%d, size:%012llx, offset:%012llx, disk->disk_name:%s\n",
        __PRETTY_FUNCTION__, __LINE__, part_num, size, offset, disk->disk_name);
...
#ifdef CONFIG_OFFS_FS_SUPPORT
	extern int mmc_offs_probe(struct mmc_card *card);
	extern void mmc_offs_remove(struct mmc_card *card);
	extern int mmc_offs_init(void);
	extern void mmc_offs_exit(void);
#endif
