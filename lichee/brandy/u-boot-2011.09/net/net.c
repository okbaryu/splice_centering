...
//#define  NET_UPGRADE_DEBUG2 1
#ifdef TFTP_UPGRADE_DEBUG2
#define NET_DEBUG2(fmt,args...)	printf(fmt ,##args)
#else
#define NET_DEBUG2(fmt,args...) do {} while(0)
#endif
...
IPaddr_t	NetOurIP;
IPaddr_t    g_Update_NetOurIP ;
/* Server IP addr (0 = unknown) */
IPaddr_t	NetServerIP;

IPaddr_t    g_NetServerIP ;
...
NetInitLoop(proto_t protocol)
{
  ...
  printf("NetInitLoop NetOurIP = 0x %lx\n", NetOurIP );
	return;
}



//static   ulong g_dhcp_cnt  = 0;
...
{
    static   ulong g_dhcp_cnt  = 0;
	static   ulong g_tftp_cnt  = 0;
	static   ulong g_tftp_max_cnt = 90000000; 
	static uint dhcp_start = 0;
	static uint tftp_start = 0;
	bd_t *bd = gd->bd;
    ...
    printf("before eth_halt in NetLoop\n");
	eth_halt();
	printf("after eth_halt in NetLoop\n");
#ifdef CONFIG_NET_MULTI
	eth_set_current();
#endif
	if (eth_init(bd) < 0) {
...
#endif
//    	if ((g_tftp_cnt > 600000)||(g_dhcp_cnt > 600000)||dhcp_netupdate_status==1) {
		if (( g_tftp_cnt > g_tftp_max_cnt )||(g_dhcp_cnt > 240865000)||dhcp_netupdate_status==1) {
     		printf("waiting...,g_dhcp_cnt=%lx,g_tftp_cnt=%lx\n",g_dhcp_cnt,g_tftp_cnt);
			dhcp_start = 0;
			g_dhcp_cnt = 0;
			tftp_start = 0;
			g_tftp_cnt = 0;
			printf("dhcp_netupdate_status = %d\n",dhcp_netupdate_status);
			eth_halt();
			return -1;
		}
        ArpTimeoutCheck();
		 ...
		 */
		 if (timeHandler && ((get_timer(0) - timeStart) > timeDelta)) {
			printf("get_timer(0) = %lx,timeStart = %lx,timeDelta = %lx\n",get_timer(0),timeStart,timeDelta);
            thand_f *x;
...
NetSendUDPPacket(uchar *ether, IPaddr_t dest, int dport, int sport, int len)
{
	...
	if (memcmp(ether, NetEtherNullAddr, 6) == 0) {
		...
		NET_DEBUG2("sending ARP for %08lx\n", dest);
...
NetReceive(volatile uchar *inpkt, int len)
{
	...
	NET_DEBUG2("##Receive from protocol 0x%x\n", x);
	...
	case PROT_ARP:
		...
		case ARPOP_REQUEST:
			...
			NET_DEBUG2("Got ARP REQUEST, return our IP\n");
			pkt = (uchar *)et;
		...
		case ARPOP_REPLY:		/* arp reply */
			...
			debug("=>Got ARP REPLY, set server/gtwy eth addr (%pM)\n",
				arp->ar_data);
			NET_DEBUG2("=>Got ARP REPLY, set server/gtwy eth addr (%pM)\n",
						arp->ar_data);
