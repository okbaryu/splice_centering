...
unsigned int dhcp_netupdate_status = 0;
char g_config_down_load = 0;
#define BOOTP_VENDOR_MAGIC	0x63825363	/* RFC1048 Magic Cookie		*/

//#define TIMEOUT		5000UL	/* Milliseconds before trying BOOTP again */       // richard  debug 

#define TIMEOUT		(180000UL)  // richard add 
...
extern IPaddr_t    g_NetServerIP ;
extern unsigned int dhcp_netupdate_status ;
	    ...
	    unsigned len)
{
	...
	case REQUESTING:
		...
		if ( DhcpMessageType((u8 *)bp->bp_vend) == DHCP_ACK ) {
			...
			dhcp_state = BOUND;
			dhcp_netupdate_status = 1 ; /*dhcp ok*/

			
			printf ("DHCP client bound to address %pI4\n", &NetOurIP);

//     		g_Update_NetOurIP =  NetOurIP ; // richard debug add 
           
		    g_NetServerIP =  (NetServerIP & (0xffff0000))|(NetOurIP&(0xffff));
//			printf ("g_NetServerIP bound to address %pI4\n", &g_NetServerIP);
			//printf ("DHCP client bound to address & 0x%x\n", &NetOurIP);
