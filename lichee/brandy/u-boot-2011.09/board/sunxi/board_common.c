...
int check_android_misc(void)
{
	...
	memset(boot_command, 0, sizeof(boot_command));

	if (NULL == strstr(getenv("bootcmd"), "recovery")) {
		set_boot_type_cmd(boot_command);

		setenv("bootcmd", boot_command);
	}
	...
	if(!strcmp(misc_message->command, "boot-recovery"))
	{
		...
		sunxi_str_replace(boot_command, "boot_normal", "boot_recovery");
	...
	else if(!strcmp(misc_message->command, "bootloader"))
	{
		...
		sunxi_str_replace(boot_command, "boot_normal", "boot_fastboot");
			sunxi_flash_write(misc_offset, 2048/512, misc_fill);
	}
...
************************************************************************************************************
*/
static void set_boot_type_cmd(char *boot_command)
{
	if (uboot_spare_head.boot_data.storage_type == 0 || uboot_spare_head.boot_data.storage_type == 5)
	{
		strcpy(boot_command, "run setargs_nand boot_normal");
	}
	else if (uboot_spare_head.boot_data.storage_type == 3)
	{
		strcpy(boot_command, "run setargs_spinor boot_normal");
	}
	else
	{
		strcpy(boot_command, "run setargs_mmc boot_normal");
	}
	//判断存储介质
	if ((uboot_spare_head.boot_data.storage_type == 1) || (uboot_spare_head.boot_data.storage_type == 2))
	{
		sunxi_str_replace(boot_command, "setargs_nand", "setargs_mmc");
	}
}
/*
************************************************************************************************************
...
{
	user_gpio_set_t power_hold_cfg;
	if (0 == script_parser_fetch("gpio_pins", "power_hold", (void *)&power_hold_cfg, sizeof(power_hold_cfg)/sizeof(int))) {
		u32 power_hold;

		printf("Set power hold to %s\n", power_hold_cfg.data ? "hi" : "lo");

		if ((power_hold = gpio_request(&power_hold_cfg, 1))) {
			gpio_write_one_pin_value(power_hold, power_hold_cfg.data, "power_hold");
		}
	}
