...
/*generate mbr info for cmdline like this env@[start:size]rootfs@[start:size]*/
void generate_format_mbr_string(char *result)
{
	sunxi_mbr_t *mbr_info = mbr_buf;
	sunxi_partition	*part_info;
	char tmp_buf[32]={0};
	int i;
	if(result == NULL)
	{
		printf("result point is null\n");
		return;
	}
	if(!mbr_status)
	{
		printf("mbr does't exist\n");
		return;
	}
	for(part_info = mbr_info->array, i=0;i<mbr_info->PartCount;i++, part_info++)
	{
		sprintf(tmp_buf,"%s@[%d:%d]",part_info->name,part_info->addrlo,part_info->lenlo);
