...
extern uint get_vertical_display(void);
...
int sprite_cartoon_screen_set(void)
{
...
#else
	/* 初始化图形参数 */
	sprite_source.screen_width  = board_display_get_screen_width(); 
	sprite_source.screen_height = board_display_get_screen_height();
	...
	// boot_logo를 복사한다.
	{
		...
		buffer = (__u8 *)(CONFIG_SYS_SDRAM_BASE + gd->ram_size - SUNXI_DISPLAY_FRAME_BUFFER_SIZE);
#else
		buffer = (__u8 *)SUNXI_DISPLAY_FRAME_BUFFER_ADDR;
#endif
		...
		if ((bmp->header.signature[0] != 'B') || (bmp->header.signature[1] != 'M')) {
			...
			memcpy(sprite_source.screen_buf, buffer + bmp->header.data_offset, sprite_source.screen_size);
		}
	}
...
int sprite_cartoon_test(void)
{
	...
	screen_width  = board_display_get_screen_width();
	screen_height = board_display_get_screen_height();
...
uint sprite_cartoon_create(void)
{
	...
	if (!get_vertical_display()) {
		...
		y1 = screen_height * 3 / 4 - 20;
		y2 = screen_height * 3 / 4 + 20;
	} else {
		screen_width  = board_display_get_screen_height();
		screen_height = board_display_get_screen_width();

		y1 = screen_width/(screen_width/10);
		y2 = y1 * (screen_width/10-1);

		x1 = screen_height / 2 - 20;
		x2 = screen_height / 2 + 20;
	}
	printf("bar x1: %d y1: %d\n", x1, y1);
	printf("bar x2: %d y2: %d\n", x2, y2);
	printf("screen_width = %d\n", screen_width);
	printf("screen_height = %d\n", screen_height);
	progressbar_hd = sprite_cartoon_progressbar_create(x1, y1, x2, y2);
	sprite_cartoon_progressbar_config(progressbar_hd, SPRITE_CARTOON_GUI_LIGHTGRAY, SPRITE_CARTOON_GUI_LIGHTGREEN, 2);
	sprite_cartoon_progressbar_active(progressbar_hd);
	flush_cache((unsigned long)sprite_source.screen_buf, screen_width * screen_height * 4);
	sprite_uichar_init(24);
