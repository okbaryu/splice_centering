...
************************************************************************************************************
*/
uint get_vertical_display(void)
{
	if (!script_parser_fetch("card_display","vertical_display",&vertical_display,1)) {
		printf("[debug_mary]vertical_display = %d\n", vertical_display);
...
uint sprite_cartoon_progressbar_create(int x1, int y1, int x2, int y2)
{
	...
	progress->height = x2 - x1;
	progress->width  = y2 - y1;
	}
