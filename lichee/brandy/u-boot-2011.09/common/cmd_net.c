...
{
	char tmp[22];
	printf("netboot_update_env NetOurIP ?\n");
...
int
tftp_netboot_common (proto_t proto, cmd_tbl_t *cmdtp, int argc, char * const arg1,char *const arg2)
{
	char *s;
//	char *end;
//	int   rcode = 0;
...
//	printf(" netboot_common load_addr=[0x%x]\n",load_addr);
	...
	case 3:	load_addr = simple_strtoul(arg1, NULL, 16);
		copy_filename (BootFile, arg2, sizeof(BootFile));
...
//		show_boot_progress (-81);
       netboot_update_env();     // richard debug add 
		return -1;
	}

//	show_boot_progress (81);
	/* NetLoop ok, update environment */
//    printf("before flush cache \n");
	netboot_update_env();    // richard debug add 
//    printf("after flush cache \n");
...
//		show_boot_progress (-82);
	...
	/* flush cache */
#if  0  // debug richard liu
	flush_cache(load_addr, size);

    printf("b  show_boot_progress\n");
	show_boot_progress(82);
	printf("b  bootm_maybe_autostart\n");
	rcode = bootm_maybe_autostart(cmdtp, argv[0]);
   printf("a  bootm_maybe_autostart\n");
	if (rcode < 0)
...
netboot_common (proto_t proto, cmd_tbl_t *cmdtp, int argc, char * const argv[])
{
	...
	debug(" netboot_common load_addr=[0x%x]\n",load_addr);
