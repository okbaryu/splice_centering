...
static int  nand_open_times;
 ...
 int spinand_get_mbr(char* buffer, uint len)
{
	int i,j;
...
int spinand_uboot_init(int boot_mode)
{
	if (0 == nand_open_times++)
	{
...
int spinand_uboot_exit(int force)
{
	if(!nand_open_times)
    {
...
uint spinand_uboot_read(uint start, uint sectors, void *buffer)
{
    int ret;
...
uint spinand_uboot_write(uint start, uint sectors, void *buffer)
{
    int ret;
...
int spinand_download_boot0(uint length, void *buffer)
{
	int ret;
...
int spinand_download_uboot(uint length, void *buffer)
{
	int ret;
...
int spinand_force_download_uboot(uint length,void *buffer)
{
    int ret = 0;
...
int spinand_uboot_erase(int user_erase)
{
    return NAND_Uboot_Erase(user_erase);
}


uint spinand_uboot_get_flash_info(void *buffer, uint length)
{
    return NAND_GetParam_store(buffer, length);
}

uint spinand_uboot_set_flash_info(void *buffer, uint length)
{
    return 0;
}

uint spinand_uboot_get_flash_size(void)
{
    return get_nftl_cap();
}

int spinand_uboot_flush(void)
{
