...
#define NAND_UBOOT_BLK_CNT		18	// Keep 3 u-boot copies. 6 blocks per one copy.
...
int NAND_VersionCheck(void)
{
	...
	********************************************************************************/
	...
	for (block_index=2;block_index<7;block_index++)
	{
		...
		{
			printf("NAND: Media version @ block %u is %x %x %x %x\n", block_index, oob_buf[0], oob_buf[1], oob_buf[2], oob_buf[3]);
			if (oob_buf[2] == nand_version[2])
			{
				printf("NAND: driver version matches at block %u.\n",block_index);
				version_match_flag = 0;
				break;
			}
			else
			{
				printf("NAND: driver version mismatch at block %u.\n",block_index);
...
int NAND_EraseBootBlocks(void)
{
	...
	printf("NAND: Ready to erase boot blocks.\n");
	...
	printf("NAND: has cleared the boot blocks.\n");
...
int SPINAND_Uboot_Force_Erase(void)
{
	printf("force erase\n");
	if(NAND_PhyInit())
	{
