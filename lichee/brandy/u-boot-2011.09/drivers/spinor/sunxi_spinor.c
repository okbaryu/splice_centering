...
struct protect_par_addr
{
	unsigned  int       start_block;// ==addrlo
	unsigned  int       index_cnt;// ==lenlo*512/(NOR_ERASE_SECTOR)
};
...
int spinor_datafinish_card(void)
{
	...
	uint index_cnt   = 0;
	uint start_block = 0;
	uchar  img_mbr[1024 * 1024];//sunxi_mbr_t
	sunxi_partition  		*part_info;
	struct protect_par_addr protect_par[8];
	int num_protect_par = 0;
	...
	printf("spinor id = 0x%x\n", id);
	//fetch user config: private: keydata
	...
	for(part_info = ((sunxi_mbr_t *)img_mbr)->array, i=0;i<((sunxi_mbr_t *)img_mbr)->PartCount;i++, part_info++)
	{
		if(0x8000==part_info->keydata)
		{
			protect_par[num_protect_par].start_block = sunxi_partition_get_offset_byname(part_info->name);
			protect_par[num_protect_par].index_cnt = sunxi_partition_get_size_byname(part_info->name)*512/(NOR_ERASE_SECTOR);
			num_protect_par++;
			printf("detected %s partition to be protected!\n",part_info->name);
		}
	}
	if(num_protect_par>0)
	{
		printf("start to erase by block.\n");
		uint break_num;
		for ( i = 0;i< total_write_bytes/(NOR_ERASE_SECTOR); i++)
		{
			for(break_num = 0;break_num++;break_num<num_protect_par)// if @ the keydata protect partition, break to erase
			{
				if(i>=protect_par[break_num].start_block&&i<protect_par[break_num].start_block+protect_par[break_num].index_cnt)
				break;
			}
			spinor_erase_one_block(i);
			if(i%2)
			sprite_cartoon_upgrade((total_count++*100)/(195+21*2));
		}
		total_count = total_count-21;
		printf("count = %d \n",total_count);
		for(break_num = num_protect_par;break_num--;break_num>0)// if @ the keydata protect partition, reset write buffer
		{
			memset(spinor_store_buffer+(protect_par[break_num].start_block-1)*512,'/0',protect_par[break_num].index_cnt*512/8);
		}
	}else{
