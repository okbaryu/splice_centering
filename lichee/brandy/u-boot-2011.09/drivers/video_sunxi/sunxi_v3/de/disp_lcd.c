...
//lcd_power0
	for(i=0; i<LCD_POWER_NUM; i++)
	{
		...
		ret = disp_sys_script_get_item(primary_key,sub_name, (int *)(&tmp), sizeof(tmp)/sizeof(int));
		if(ret == 3) {
			/* gpio */
			lcd_cfg->lcd_power_type[i] = 1; /* gpio */
			memcpy(&(lcd_cfg->lcd_power[i]), &tmp, sizeof(disp_gpio_set_t));
		} else if(ret == 2) {
			/* str */
			memcpy(&(lcd_cfg->lcd_regu[i][0]), &tmp, 25);
...
//backlight adjust
	...
	ret = disp_sys_script_get_item(primary_key, "lcd_rotate_boot", &value, 1);
	if (ret < 0) {
