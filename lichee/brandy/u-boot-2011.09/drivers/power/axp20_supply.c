...
static int axp20_set_gpio_output(int sppply_index, int low)
{
	u8   reg_value;
	switch(sppply_index)
	{
		...
		case 2:
		{
			printf("mary_dbg:axp20_set_gpio_output() set pmu gpio2\n");
			if(axp_i2c_read(AXP20_ADDR, BOOT_POWER20_GPIO2_CTL, &reg_value))
			{
				return -1;
		    }
		    if(low)
			...
			if(axp_i2c_write(AXP20_ADDR, BOOT_POWER20_GPIO2_CTL, reg_value))
			{
				printf("sunxi pmu error : unable to set POWER20_GPIO2\n");
...
int axp20_set_supply_status(int vol_name, int vol_value, int onoff)
{
	...
	switch(supply_type)
	{
		...
		case PMU_SUPPLY_GPIO_TYPE:
			return axp20_set_gpio_output(sppply_index, onoff);
