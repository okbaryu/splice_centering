...
//				nand_uboot_get_flash_info((void *)uboot->boot_data.nand_spare_data, STORAGE_BUFFER_SIZE);
			...
			#endif
            if(production_media == 5)
            {
                spinand_uboot_get_flash_info((void *)uboot->boot_data.nand_spare_data, STORAGE_BUFFER_SIZE);
            }
		}
...
//	        return nand_download_uboot(uboot->boot_head.length, buffer);
        ...
        if(production_media == 5)
        {   
            debug("spinand down uboot\n");
            if(uboot_spare_head.boot_data.work_mode == WORK_MODE_BOOT)
            {   
                printf("work_mode_boot \n");
                return spinand_force_download_uboot(uboot->boot_head.length,buffer);
            }
            else
            {   
		        //uboot->boot_data.storage_type = 5;
                return spinand_download_uboot(uboot->boot_head.length, buffer);
            }
        }
		else
		{
...
//				return nand_download_uboot(toc1->valid_len, buffer);
        ...
        {   
            ...
            {   
                printf("work_mode_boot\n");
                return spinand_force_download_uboot(toc1->valid_len,buffer);
            }
            else
            {   
                return spinand_download_uboot(toc1->valid_len, buffer);
            }
        }
		else
		{
...
int sunxi_sprite_download_boot0(void *buffer, int production_media)
{
	if(!gd->securemode)
    {
        ...
        if(production_media == 5)
        {
            spinand_uboot_get_flash_info((void *)boot0->prvt_head.storage_data, STORAGE_BUFFER_SIZE);
        }
        else{
        ...
        if(production_media == 5)
        {
            return spinand_download_boot0(boot0->boot_head.length, buffer);
        }
		else
		{
	...
	else
	{
		...
		if(production_media  == 5)
		{
			spinand_uboot_get_flash_info((void *)toc0_config->storage_data, STORAGE_BUFFER_SIZE);
		}
		else{
			extern int mmc_write_info(int dev_num,void *buffer,u32 buffer_size);
        ...
        if(production_media == 5)
        {
            return spinand_download_boot0(toc0->length, buffer);
        }
		else
		{
