...
int sunxi_sprite_erase_flash(void  *img_mbr_buffer)
{
	...
	{
		printf("sprite: flash already erased\n");
		return 0;
	}
	...
	{
		printf("sprite: need to erase flash\n");
	}
	else
	{
		printf("sprite: do not need to erase flash\n");
		if (production_media == 5) {
			// spinand의 경우 MBR이 기록되어 있을 때 다시 기록을 시도하면 오류가 생기므로 이를 피한다.
			mbr_burned_flag = 1;
		}
		return 0;
	}
	...
	{
		printf("sprite: forced to erase flash\n");
	...
	{
		printf("sprite: no partition to preserve user data\n");
	...
	sunxi_sprite_exit(1);
	printf("sprite: need_erase_flag = %d\n", need_erase_flag);
	//开始擦除
