...
int sunxi_secstorage_read(int item, unsigned char *buf, unsigned int len)
{
	...
	else
	{
		if(uboot_spare_head.boot_data.work_mode == WORK_MODE_BOOT)
			return sunxi_flash_mmc_secread(item, buf, (len+511)/512);
		else
			return sunxi_sprite_mmc_secread(item,buf, (len+511)/512);
	}
}
/*
************************************************************************************************************
...
int sunxi_secstorage_write(int item, unsigned char *buf, unsigned int len)
{
	...
	{
		if(uboot_spare_head.boot_data.work_mode == WORK_MODE_BOOT)
			return sunxi_flash_mmc_secwrite(item, buf, (len+511)/512);
		else
			return sunxi_sprite_mmc_secwrite(item,buf, (len+511)/512);
	}
}
/*
************************************************************************************************************
