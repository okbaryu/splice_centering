...
unsigned int mctl_sys_init(__dram_para_t *para)
{
	...
	if((para->dram_tpr8&0x1))
	{
		...
		if(normal_mode)
			  ...
			  ret_val=ccm_set_pll_ddr_clk(720);
		m_div = 1;
	...
	}else
	{
		...
		if(normal_mode)
				...
				ret_val=ccm_set_pll_ddr_clk(360);
		m_div = 2;
		dram_dbg("PLL FREQUENCE = %d MHZ\n",ret_val);
	}
	//set PLL SSCG
...
//	while((mctl_read_w(PGSR0 )&0x1) != 0x1);
	if(normal_mode)
	{
			...
			mctl_write_w(0x368ED9BA,ZQnDR(0));
			mctl_write_w(0x31A399BA,ZQnDR(1));
			if((para->dram_tpr8&0x1))	//PLL bypass
			{
					if((para->dram_type) == 3)
						reg_val = 0x000201f1;
...
signed int init_DRAM(int type, __dram_para_t *para)
{
	...
	if(readl(CONFIG_SBROMSW_BASE)!=0xffff)
		normal_mode=0;
	printf("DRAM DRIVE INFO: V0.9 \n");
	printf("DRAM Type =%d (2:DDR2,3:DDR3,6:LPDDR2,7:LPDDR3)\n", para->dram_type);
	printf("DRAM zq value: %x",para->dram_zq);
	printf("DRAM normal_mode value: %x",normal_mode);
	if(normal_mode)
	{
