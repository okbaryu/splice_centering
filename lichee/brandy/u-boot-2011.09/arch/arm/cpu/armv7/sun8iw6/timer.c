...
#define  TIMER_TICK_PER_MS(time)    (((time) * 128)/24/1000)

static void __msdelay_nssecure(__u32 ms);
static void __usdelay_nssecure(__u32 us);
static void __msdelay_secure(__u32 ms);
static void __usdelay_secure(__u32 us);
static void __delay_null(u32 time);

void (* __msdelay_pt )(u32 ms) = __delay_null;
void (* __usdelay_pt )(u32 us) = __delay_null;
/*
************************************************************************************************************
*
*                                             function
...
static void __msdelay_nssecure(__u32 ms)
{
	...
	writel(reg_val, TMRC_CTRL(1));
	writel(ms * 3000, TMRC_INTV(1));
	reg_val = 0;
	do
	{
		reg_val = readl(TMRC_CTRL(1));
	}
	while(reg_val & 0x02);
	writel(readl(TMRC_CTRL(1)) | 1, TMRC_CTRL(1));

	while(!(readl(TMRC_INT_ST) & (1<<1)));
	writel(1<<1, TMRC_INT_ST);
...
void timer_exit(void)
{
    ...
    writel(readl(CCM_AVS_SCLK_CTRL) & 0x0fffffff, CCM_AVS_SCLK_CTRL);
...
{
#ifdef CONFIG_SUNXI_SECURE_SYSTEM
	if(__usdelay_pt == __usdelay_nssecure)
	{
		uint time;

		time = 0xffffffff - readl(TMRC_CURNT(0));
		time = TIMER_TICK_PER_MS(time);
