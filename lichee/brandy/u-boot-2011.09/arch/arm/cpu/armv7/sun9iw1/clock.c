...
int sunxi_clock_get_corepll(void)
{
    ...
    if(get_cluster_id() == 0)
        ...
        reg_val = readl(CCM_PLL2_C1_CTRL);
...
static int clk_set_pll2_para(int frequency, int core_vol)
{
	...
	writel(reg_val, CCM_PLL2_C1_CTRL);
...
int sunxi_clock_set_corepll(int frequency, int core_vol)
{
    ...
    if(frequency != 24)
    {
		...
		if(frequency>1600)
		{
		    writel(0x303, CCM_CLUSTER0_AXI_RATIO);
		}
		else if(frequency>800)
		{
		    writel(0x302, CCM_CLUSTER0_AXI_RATIO);
		}
...
int sunxi_clock_set_C1corepll(int frequency, int core_vol)
{
    ...
    //set cpuB vol
    axp_set_supply_status_byname("axp806","dcdca_vol",core_vol,1);
    core_vol += 1000000;
    ...
    if(frequency != 24)
    {
		...
		if(frequency>1600)
		{
		    writel(0x303, CCM_CLUSTER1_AXI_RATIO);
		}
		else if(frequency>800)
		{
		    writel(0x302, CCM_CLUSTER1_AXI_RATIO);
		}
		else if(frequency>400)
		{
		    writel(0x101, CCM_CLUSTER1_AXI_RATIO);
		}
		//切换时钟到COREPLL上
	    reg_val = readl(CCM_CPU_SOURCECTRL);
	    reg_val |= 0x01<<8;
